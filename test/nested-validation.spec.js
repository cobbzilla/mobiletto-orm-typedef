...
describe("nested validation test with optional nested object with required fields", async () => {
  it("successfully sets tabIndexes for typeDef and all nested objects", async () => {
    ...
    expect(nestedType1.fields.nestedObject.tabIndexes[2]).eq(
      "triplyNestedObject"
    );
    expect(
      nestedType1.fields.nestedObject.fields.triplyNestedObject.tabIndexes
        .length
    ).eq(2);
    expect(
      nestedType1.fields.nestedObject.fields.triplyNestedObject.tabIndexes[0]
    ).eq("nested3Required");
    expect(
      nestedType1.fields.nestedObject.fields.triplyNestedObject.tabIndexes[1]
    ).eq("nested3Plain");
  });
  ...
  it("successfully validates an empty object against a typedef with an optional nested object with required fields", async () => {
    try {
      ...
      expect(e.errors["id"][0]).equals(
        "required",
        "expected id.required error"
      );
      expect(e.errors["primaryField"].length).equals(
        1,
        "expected 1 primary error"
      );
      expect(e.errors["primaryField"][0]).equals(
        "required",
        "expected primary.required error"
      );
    }
  });
});
...
describe("nested validation test with required nested object with required fields", async () => {
  it("successfully validates an empty object against this complex typedef with a required nested object with required fields", async () => {
    try {
      ...
      expect(e.errors["nestedObject"].length).equals(
        1,
        "expected 1 nestedObject error"
      );
      expect(e.errors["nestedObject"][0]).equals(
        "required",
        "expected nestedObject.required error"
      );
    }
  });
  it("successfully validates an object with an empty nested value against this complex typedef", async () => {
    try {
      ...
      expect(e.errors["nestedObject.nested1"].length).equals(
        1,
        "expected 1 nestedObject.nested1 error"
      );
  ...
  it("successfully validates an object with many invalid values against this complex typedef", async () => {
    try {
      ...
      expect(e.errors["nestedObject.nested2"].length).equals(
        1,
        "expected 1 nestedObject.nested2 error"
      );
      expect(e.errors["nestedObject.nested2"][0]).equals(
        "regex",
        "expected nestedObject.nested2.regex error"
      );
      expect(
        e.errors["nestedObject.triplyNestedObject.nested3Plain"].length
      ).equals(
        1,
        "expected 1 nestedObject.triplyNestedObject.nested3Plain error"
      );
      expect(
        e.errors["nestedObject.triplyNestedObject.nested3Plain"][0]
      ).equals(
        "values",
        "expected nestedObject.triplyNestedObject.nested3Plain.values error"
      );
      expect(
        e.errors["nestedObject.triplyNestedObject.nested3Required"].length
      ).equals(
        1,
        "expected 1 nestedObject.triplyNestedObject.nested3Required error"
      );
      expect(
        e.errors["nestedObject.triplyNestedObject.nested3Required"][0]
      ).equals(
        "required",
