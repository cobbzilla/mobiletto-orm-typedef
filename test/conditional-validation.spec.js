...
describe("validation test with conditionally required nested objects", async () => {
  ...
  it("successfully validates a thing that selects from selector but does not supply any values", async () => {
    try {
      ...
      expect(e.errors["nested1"][0]).equals(
        "required",
        "expected nested1.required error"
      );
    }
  });
  it("successfully validates a thing that selects 1 from selector but does not supply appropriate values", async () => {
    try {
      ...
      expect(Object.keys(e.errors).length).equals(2, "expected two errors");
      expect(e.errors["nested1.field1"].length).equals(
        1,
        "expected 1 nested1.field1 error"
      );
      expect(e.errors["nested1.field1"][0]).equals(
        "required",
        "expected nested1.field1.required error"
      );
      expect(e.errors["nested1.field2"].length).equals(
        1,
        "expected 1 nested1.field1 error"
      );
      expect(e.errors["nested1.field2"][0]).equals(
        "required",
        "expected nested1.field1.required error"
      );
    }
  ...
  it("successfully validates a thing that selects 2 from selector but does not supply appropriate values", async () => {
    try {
      ...
      expect(Object.keys(e.errors).length).equals(2, "expected two errors");
      expect(e.errors["nested2.field1"].length).equals(
        1,
        "expected 1 nested2.field1 error"
      );
      expect(e.errors["nested2.field1"][0]).equals(
        "required",
        "expected nested2.field1.required error"
      );
      expect(e.errors["nested2.field3"].length).equals(
        1,
        "expected 1 nested2.field3 error"
      );
      expect(e.errors["nested2.field3"][0]).equals(
        "required",
        "expected nested2.field3.required error"
      );
    }
  });
