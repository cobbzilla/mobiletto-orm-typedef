...
export const determineFieldType = (fieldName, field) => {
    ...
    if (typeof field.min === "number" ||
        ...
        if (foundType != null && foundType !== "string" && foundType !== "string[]") {
            throw new MobilettoOrmError(`invalid(1) TypeDefConfig: field ${fieldName} had incompatible types: ${foundType} / string`);
        }
    ...
    if (typeof field.minValue === "number" ||
        ...
        if (foundType != null && foundType !== "number" && foundType !== "number[]") {
            throw new MobilettoOrmError(`invalid(2) TypeDefConfig: field ${fieldName} had incompatible types: ${foundType} / number`);
        }
    ...
    if (defaultType !== "undefined") {
        if (field.type && isArrayType(field.type)) {
            if (!Array.isArray(field.default)) {
                throw new MobilettoOrmError(`invalid(3) TypeDefConfig: field ${fieldName} had type 'array' or control 'multi' but default value type is ${defaultType} (expected array)`);
            }
        ...
        if (defaultType) {
            if (foundType != null && foundType !== defaultType) {
                throw new MobilettoOrmError(`invalid(4) TypeDefConfig: field ${fieldName} had incompatible types: ${foundType} / ${defaultType}`);
            }
    ...
    if (hasValues || hasItems) {
        ...
        if (hasValues && hasItems && values.length !== items.length) {
            throw new MobilettoOrmError(`invalid(5) TypeDefConfig: field ${fieldName} had different lengths for values (${values.length}) vs items (${items.length})`);
        }
        if (hasLabels) {
            /* eslint-disable @typescript-eslint/no-non-null-assertion */
            ...
            if (hasValues && values.length !== labels.length) {
                throw new MobilettoOrmError(`invalid(6) TypeDefConfig: field ${fieldName} had different lengths for values (${values.length}) vs labels (${labels.length})`);
            }
            if (hasItems && items.length !== labels.length) {
                throw new MobilettoOrmError(`invalid(7) TypeDefConfig: field ${fieldName} had different lengths for items (${values.length}) vs labels (${labels.length})`);
            }
        }
        if (!isMulti && !isArrayType(field.type)) {
            ...
            if (vType) {
                if (foundType != null && foundType !== vType) {
                    throw new MobilettoOrmError(`invalid(8) TypeDefConfig: field ${fieldName} had incompatible types: ${foundType} / ${vType}`);
                }
                foundType = vType;
            }
        }
    ...
    if (hasFields) {
        if (foundType != null && foundType !== "object" && foundType !== "object[]") {
            throw new MobilettoOrmError(`invalid(9) TypeDefConfig: field ${fieldName} had incompatible types: ${foundType} / object`);
        }
        foundType = foundType || "object";
    }
    if (foundType) {
        if (field.type && foundType !== field.type) {
            throw new MobilettoOrmError(`invalid(10) TypeDefConfig: field ${fieldName} had type: ${field.type} but discovered type was ${foundType}`);
        }
