...
import { AUTO_REDACT_CONTROLS, DEFAULT_FIELD_INDEX_LEVELS, NUMERIC_CONTROL_TYPES, RESERVED_FIELD_NAMES, } from "./constants.js";
...
const determineFieldType = (fieldName, field) => {
    ...
    if (defaultType !== "undefined") {
        if (Array.isArray(field.default) && !hasValues && !hasItems) {
            throw new MobilettoOrmError(`invalid TypeDefConfig: field ${fieldName} had an array as 'default' value, but has no 'items' or 'values'`);
        }
...
export const processFields = (fields, objPath, typeDef) => {
    ...
    Object.keys(fields).forEach((fieldName) => {
        ...
        if (typeof field.primary === "boolean" && field.primary === true) {
            ...
            if (field.ref) {
                throw new MobilettoOrmError(`${invalidPrefix} primary field ${typeDef.primary} had {ref} (not allowed)`);
            }
            field.required = true;
            field.unique = true;
        ...
        if (typeof field.ref === "object") {
            if (!typeDef.registry) {
                throw new MobilettoOrmError(`${invalidPrefix} reference field ${fieldName} defined but typeDef ${typeDef.typeName} had no registry (not allowed)`);
            }
            if (typeDef.registry.strict) {
                const refType = field.ref.refType ? field.ref.refType : fieldName;
                if (!typeDef.registry.isRegistered(refType)) {
                    throw new MobilettoOrmError(`${invalidPrefix} no resolver defined in registry for reference field ${fieldName} (not allowed with registry.strict)`);
                }
            }
