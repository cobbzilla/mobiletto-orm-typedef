...
const validIdPrefix = (prefix) => (prefix && prefix.length >= 2) || false;
...
class MobilettoOrmTypeDef {
    constructor(config) {
        ...
        this.idPrefix = validIdPrefix(config.idPrefix) ? config.idPrefix : defaultIdPrefix(this.typeName);
        ...
        this.specificPathRegex = new RegExp(`^${this.typeName}_.+?${constants_js_1.OBJ_ID_SEP}_\\d{13,}_[A-Z\\d]{${util_js_1.MIN_ID_LENGTH},}\\.json$`, "gi");
    ...
    buildType(typeName, out) {
        return (0, builder_js_1.buildType)(typeName || this.typeName, this.fields, out);
    }
    newId() {
        return (0, util_js_1.generateId)(this.idPrefix);
    }
    newVersion() {
        return (0, util_js_1.generateId)("v_" + this.idPrefix);
    }
    newMeta(id) {
        const now = Date.now();
        return {
    ...
    specificBasename(obj) {
        ...
        return this.typeName + "_" + obj.id + constants_js_1.OBJ_ID_SEP + obj._meta.version + ".json";
    }
    idFromPath(p) {
        // start with basename
    ...
    tombstone(thing) {
        ...
        return {
            _meta: {
                id: thing._meta.id,
                version: (0, util_js_1.generateId)(),
                removed: true,
                ctime: thing._meta.ctime,
                mtime: Date.now(),
            },
