"use strict";
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.rand = exports.MIN_ID_LENGTH = exports.generateId = exports.sha = exports.fsSafeName = void 0;
const shasum_1 = __importDefault(require("shasum"));
const uuid_1 = require("uuid");
const fsSafeName = (name) => encodeURIComponent(name).replace(/%/g, "~");
exports.fsSafeName = fsSafeName;
const sha = (val) => (0, shasum_1.default)(val, "SHA256");
exports.sha = sha;
const generateId = (prefix) => `${prefix ? prefix + "_" : ""}${Date.now().toString(16)}_${(0, uuid_1.v4)().replace("-", "")}`.toLowerCase();
exports.generateId = generateId;
exports.MIN_ID_LENGTH = (0, exports.generateId)().length;
const rand = (len = 10) => {
    let s = "";
    while (s.length < len) {
        s += (0, exports.sha)((0, exports.generateId)());
    }
    return s.substring(0, len);
};
exports.rand = rand;
