...
exports.processFields = exports.FIELD_NAME_REGEX = void 0;
...
const processFields = (fields, objPath, typeDef) => {
    ...
    Object.keys(fields).forEach((fieldName) => {
        ...
        if (typeof field.ref === "object") {
            if (!typeDef.registry) {
                throw new errors_js_1.MobilettoOrmError(`${invalidPrefix} reference field ${fieldName} defined but typeDef ${typeDef.typeName} had no registry (not allowed)`);
            }
            if (typeDef.registry.strict) {
                ...
                if (!typeDef.registry.isRegistered(refType)) {
                    throw new errors_js_1.MobilettoOrmError(`${invalidPrefix} no resolver defined in registry for reference field ${fieldName} (not allowed with registry.strict)`);
                }
            }
