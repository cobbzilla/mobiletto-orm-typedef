...
const determineFieldType = (fieldName, field) => {
    ...
    if (defaultType !== "undefined") {
        ...
        if (Array.isArray(field.default) &&
            ...
            throw new errors_js_1.MobilettoOrmError(`invalid TypeDefConfig: field ${fieldName} had an array as default value, but is not a select or multi field`);
        }
    ...
    if (hasValues || hasItems) {
        ...
        if ((!field.control || field.control !== "multi") &&
            (!field.type || field.type !== "array")) {
            const vType = hasItems &&
                items.length > 0 &&
                typeof items[0].value !== "undefined" &&
                items[0].value != null
...
const processFields = (fields, objPath, typeDef) => {
    Object.keys(fields).forEach((fieldName) => {
        ...
        if (constants_js_1.RESERVED_FIELD_NAMES.includes(fieldName)) {
            throw new errors_js_1.MobilettoOrmError(`invalid TypeDefConfig: reserved field name: ${fieldName}`);
        }
        ...
        if (typeof field.primary === "boolean" && field.primary === true) {
            if (objPath !== "") {
                throw new errors_js_1.MobilettoOrmError(`invalid TypeDefConfig: non-root field ${fieldName} had {primary: true} (not allowed)`);
            }
            if (typeDef.primary) {
                throw new errors_js_1.MobilettoOrmError(`invalid TypeDefConfig: multiple fields had {primary: true}: ${typeDef.primary} and ${fieldName}`);
            }
            if (!field_js_1.VALID_PRIMARY_TYPES.includes(field.type)) {
                throw new errors_js_1.MobilettoOrmError(`invalid TypeDefConfig: primary ${typeDef.primary} had bad type: ${field.type}`);
            }
            typeDef.primary = fieldName;
            if (typeof field.required === "boolean" && field.required === false) {
                throw new errors_js_1.MobilettoOrmError(`invalid TypeDefConfig: primary field ${typeDef.primary} had {required: false} (not allowed)`);
            }
            field.required = true;
            if (typeof field.updatable === "boolean" && field.updatable === true) {
                throw new errors_js_1.MobilettoOrmError(`invalid TypeDefConfig: primary field ${typeDef.primary} had {updatable: true} (not allowed)`);
            }
            if (field.when) {
                throw new errors_js_1.MobilettoOrmError(`invalid TypeDefConfig: primary field ${typeDef.primary} had {when} (not allowed)`);
            }
            field.updatable = false;
        }
        if (field.index) {
            if (!typeDef) {
                throw new errors_js_1.MobilettoOrmError(`invalid TypeDefConfig: non-root field ${fieldName} had {index: true} (not allowed)`);
            }
            typeDef.indexes.push(fieldName);
        }
        const redact = (typeof field.redact === "undefined" &&
            constants_js_1.AUTO_REDACT_CONTROLS.includes(field.control)) ||
            (typeof field.redact === "boolean" && field.redact === true);
        if (redact) {
            if (fieldName === "id") {
                throw new errors_js_1.MobilettoOrmError(`invalid TypeDefConfig: {redact: true} not allowed on id field`);
            }
            field.redact = true;
            typeDef.redaction.push(fieldPath);
        }
        if (field.values && Array.isArray(field.values)) {
            const hasLabels = field.labels &&
                Array.isArray(field.labels) &&
                field.labels.length === field.values.length;
            field.items = [];
            if (!hasLabels)
                field.labels = [];
