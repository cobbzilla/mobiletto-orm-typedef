...
export const FIELD_VALIDATORS: FieldValidators = {
  ...
  min: (val: any, limit: number): boolean =>
    val == null || (typeof val === "string" && val.length >= limit),

  max: (val: any, limit: number): boolean =>
    val == null || (typeof val === "string" && val.length <= limit),

  minValue: (val: any, limit: number): boolean =>
    val == null || (typeof val === "number" && val >= limit),

  maxValue: (val: any, limit: number): boolean =>
    val == null || (typeof val === "number" && val <= limit),
  ...
  for (const fieldName of Object.keys(fields)) {
    ...
    if (field.type === "object") {
      if (field.required && thingValueType !== "object") {
        ...
        validated[fieldName] = {};
        const currentValue =
          current && typeof current === "object" && current[fieldName]
            ? current[fieldName]
            : null;
        validateFields(
          rootThing,
    ...
    const currentValueType =
      isCreate || !current ? "undefined" : typeof current[fieldName];
    const updatable =
      typeof field.updatable === "undefined" || !!field.updatable;
    const useThingValue =
      isCreate ||
      (updatable && thingValueType !== "undefined" && thing[fieldName] != null);
