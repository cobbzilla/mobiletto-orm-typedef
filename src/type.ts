...
export class MobilettoOrmTypeDef {
  ...
  constructor(config: MobilettoOrmTypeDefConfig) {
    if (typeof config.typeName !== "string" || config.typeName.length <= 0) {
      throw new MobilettoOrmError(
        "invalid TypeDefConfig: no typeName provided"
      );
    }
    if (config.typeName.includes("%") || config.typeName.includes("~")) {
      throw new MobilettoOrmError(
        "invalid TypeDefConfig: typeName cannot contain % or ~ characters"
      );
    }
    this.config = config;
    this.alternateIdFields =
      config.alternateIdFields || DEFAULT_ALTERNATE_ID_FIELDS;
    ...
    this.validations =
      config.validations &&
      typeof config.validations === "object" &&
      Object.keys(config.validations).length > 0
        ? config.validations
        : {};
  ...
  defaultFieldValue(
    ...
    opts: MobilettoOrmDefaultFieldOpts
  ): MobilettoOrmFieldValue {
    ...
    if (field.type === "string")
      return dummy
        ? randomstring.generate(
            Math.ceil(Math.random() * (field.max ? field.max : 10))
          )
        : "";
    ...
    this.log_warn(
      `defaultFieldValue: unknown field.type=${
  ...
  async validate(thing: MobilettoOrmInstance, current: MobilettoOrmInstance) {
    ...
    if (
      ...
      thing.version.length < MIN_VERSION_STAMP_LENGTH
    ) {
  ...
  async typeDefValidations(validated: any, errors: ValidationErrors) {
    ...
    Object.keys(this.validations).forEach((vName) => {
      ...
      if (typeof v.valid !== "function" || typeof v.field !== "string") {
        throw new MobilettoOrmError(
          `validate: custom validation ${vName} lacked 'valid' function: ${JSON.stringify(
            v
          )}`
        );
      }
      validationPromises.push(
        ...
        new Promise(async (resolve) => {
          ...
          try {
            ...
            this.log_warn(
              `validate: custom validation ${vName} error: ${JSON.stringify(e)}`
            );
            const err = typeof v.error === "string" ? v.error : vName;
            addError(errors, v.field, err);
          } finally {
  ...
  _tabIndexes(fields = this.fields) {
    return Object.keys(fields).sort((f1, f2) =>
      compareTabIndexes(fields, f1, f2)
    );
  }
  ...
  typePath() {
    return (
      (this.basePath.length > 0 ? this.basePath + "/" : "") + this.typeName
    );
  }
  ...
  indexPath(field: string, value: MobilettoOrmFieldIndexableValue) {
    if (this.indexes.includes(field)) {
      ...
      throw new MobilettoOrmError(
        `typeDef.indexPath: field not indexed: ${field}`
      );
    }
  }
