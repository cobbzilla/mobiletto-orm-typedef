...
export const processFields = (fields: MobilettoOrmFieldDefConfigs, objPath: string, typeDef: MobilettoOrmTypeDef) => {
    ...
    Object.keys(fields).forEach((fieldName) => {
        ...
        if (typeof field.ref === "object") {
            if (!typeDef.registry) {
                throw new MobilettoOrmError(
                    `${invalidPrefix} reference field ${fieldName} defined but typeDef ${typeDef.typeName} had no registry (not allowed)`
                );
            }
            if (typeDef.registry.strict) {
                ...
                if (!typeDef.registry.isRegistered(refType)) {
                    throw new MobilettoOrmError(
                        `${invalidPrefix} no resolver defined in registry for reference field ${fieldName} (not allowed with registry.strict)`
                    );
                }
            }
