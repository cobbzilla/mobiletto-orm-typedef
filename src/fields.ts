...
const determineFieldType = (
  ...
  if (foundType) {
    if (!VALID_FIELD_TYPES.includes(foundType)) {
      throw new MobilettoOrmError(
        `invalid TypeDefConfig: field ${fieldName} had invalid type: ${foundType}`
      );
    }
    return foundType;
  }
...
export const processFields = (
  ...
  Object.keys(fields).forEach((fieldName) => {
    ...
    if (RESERVED_FIELD_NAMES.includes(fieldName)) {
      throw new MobilettoOrmError(
        `invalid TypeDefConfig: reserved field name: ${fieldName}`
      );
    }
    ...
    const redact =
      (typeof field.redact === "undefined" &&
        AUTO_REDACT_CONTROLS.includes(field.control)) ||
      (typeof field.redact === "boolean" && field.redact === true);
    if (redact) {
      if (fieldName === "id") {
        throw new MobilettoOrmError(
          `invalid TypeDefConfig: {redact: true} not allowed on id field`
        );
      }
      field.redact = true;
